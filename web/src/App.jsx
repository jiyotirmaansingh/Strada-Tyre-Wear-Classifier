...
const SLOTS = [
  ...
  },
  {
    ...
    label: "Right Profile",
    icon: "◨",
    hint: "Side view, right",
  },
  {
    id: "area_of_interest",
    label: "Area of Interest",
    icon: "◎",
    hint: "Focused wear zone",
  },
  {
    id: "tread_closeup",
    label: "Tread Close-up",
    icon: "▦",
    hint: "Primary analysis input",
  },
  {
    id: "cracks",
    label: "Cracks / Sidewall",
    icon: "⌁",
    hint: "Optional sidewall scan",
  },
];
...
const DARK = {
  ...
  panelShadow:
    "0 0 0 0.5px rgba(255,255,255,0.04) inset, 0 20px 60px rgba(0,0,0,0.5)",
  gridLine: "rgba(255,255,255,0.011)",
  noiseOpacity: 0,
  orbBg:
    "radial-gradient(ellipse 80% 50% at 50% 0%, rgba(249,115,22,0.09) 0%, transparent 70%)",
};

const LIGHT = {
  bg: "#faf7f4",
  surface: "rgba(255,252,249,0.99)",
  panel: "rgba(255,252,249,0.97)",
  ...
  panelShadow:
    "0 0 0 0.5px rgba(0,0,0,0.04) inset, 0 8px 40px rgba(0,0,0,0.08)",
  ...
  try {
    if (window.navigator?.vibrate) {
      window.navigator.vibrate(
        type === "light" ? 8 : type === "medium" ? 18 : 35,
      );
    }
  } catch (_) {}
}
...
const BOOT_LINES = [
  ...
  {
    delay: 1200,
    text: "All systems nominal. Launching UI →",
    color: "#f97316",
  },
];
  ...
  return (
    <div
      style={{
        ...
        overflow: "hidden",
        animation: exiting
          ? "splashFadeOut 0.5s cubic-bezier(.4,0,.2,1) forwards"
          : "none",
      }}
    >
      {/* Scanline sweep */}
      ...
      >
        ...
        <div style={{ textAlign: "center" }}>
          ...
          >
            SMART TYRE RECOGNITION & DIAGNOSTIC ASSISTANT
          </p>
        </div>
          ...
          {BOOT_LINES.map((line, i) => (
            ...
            >
              ...
              {cursorLine === i && visibleLines.includes(i) && (
                <span className="splash-cursor" style={{ color: "#f97316" }}>
                  ▋
                </span>
              )}
            </div>
    ...
    >
      ...
      <div
        style={{
          ...
          height: 6,
          background:
            "repeating-linear-gradient(90deg,rgba(0,0,0,0.18) 0px,rgba(0,0,0,0.18) 18px,transparent 18px,transparent 32px)",
...
function RacingSlickPopup({ onClose }) {
  ...
  return createPortal(
    ...
    >
      <div
        className={exiting ? "slick-popup-exit" : "slick-popup-enter"}
        style={{
          pointerEvents: "all",
          background:
            "linear-gradient(135deg,rgba(18,18,18,0.98),rgba(28,10,0,0.98))",
          ...
          width: "100%",
          boxShadow:
            "0 8px 48px rgba(239,68,68,0.25), 0 0 0 1px rgba(239,68,68,0.1)",
          cursor: "pointer",
        }}
        onClick={handleClose}
      >
        <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            >
              Our AI detects approximately{" "}
              <span style={{ color: "#f97316", fontWeight: 600 }}>
                zero tread remaining
              </span>
              . That's peak Formula 1 energy — but for a road car, that's an MOT
              fail waiting to happen. Please visit a tyre shop. Now. Like,
              immediately.
            </p>
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              <span
              ...
              >
                TAP TO DISMISS · AUTO-CLOSES IN 8s
              </span>
            </div>
          </div>
        </div>
      ...
      {[
        ...
        <div
          ...
          style={{
            ...
            height: 10,
            background:
              "repeating-linear-gradient(90deg,rgba(30,30,30,0.55) 0px,rgba(30,30,30,0.55) 14px,transparent 14px,transparent 22px)",
            borderRadius: 5,
...
function ThemeToggle({ onToggleEgg }) {
  ...
  const handleToggle = (e) => {
    ...
    const now = Date.now();
    toggleTimestamps.current = [...toggleTimestamps.current, now].filter(
      (t) => now - t < 3000,
    );
      ...
      >
        <defs>
          <radialGradient id="radarBg" cx="50%" cy="50%">
            <stop
              offset="0%"
              stopColor={
                isLight ? "rgba(255,250,245,0.97)" : "rgba(20,20,20,0.95)"
              }
            />
            <stop
              offset="100%"
              stopColor={
                isLight ? "rgba(250,247,244,0.99)" : "rgba(8,8,8,0.98)"
              }
            />
          </radialGradient>
          <filter id="glow">
        ...
        {rings.map((r, i) => (
          <circle
            ...
            r={r}
            fill="none"
            stroke={`${accent}${i === rings.length - 1 ? "22" : "11"}`}
            strokeWidth="0.8"
            strokeDasharray={i === rings.length - 1 ? "none" : "4 4"}
          />
        ))}
        {[0, 45, 90, 135].map((a) => {
          ...
          return (
            <line
              key={a}
              x1={150 + 8 * Math.cos(aR)}
              y1={150 + 8 * Math.sin(aR)}
              x2={150 + 125 * Math.cos(aR)}
              y2={150 + 125 * Math.sin(aR)}
        ...
        <g clipPath="url(#radarClip)">
          <path
            d={`M 150 150 L ${150 + 125 * Math.cos(rad - 0.52)} ${150 + 125 * Math.sin(rad - 0.52)} A 125 125 0 0 1 ${sweepX} ${sweepY} Z`}
            fill={`${accent}11`}
          />
          <path
            d={`M 150 150 L ${150 + 125 * Math.cos(rad - 0.22)} ${150 + 125 * Math.sin(rad - 0.22)} A 125 125 0 0 1 ${sweepX} ${sweepY} Z`}
            fill={`${accent}1e`}
          />
        </g>
        <line
          ...
          <g key={p.id}>
            ...
            {!isMobile && (
              <circle
                ...
                stroke={`${accent}66`}
                strokeWidth="1"
                style={{ animation: "radarPing 1.2s ease-out forwards" }}
              />
            )}
          </g>
        ))}
        <g transform="translate(150,150)">
          ...
          <ellipse
            ...
            rx="16"
            ry="16"
            fill={isLight ? "rgba(250,247,244,0.9)" : "rgba(14,14,14,0.9)"}
            stroke={`${accent}59`}
            strokeWidth="2"
          />
          ...
          <circle
            ...
            opacity="0.9"
            style={
              centerGlowing ? { filter: `drop-shadow(0 0 8px ${accent})` } : {}
            }
          />
        ...
        {Array.from({ length: 12 }).map((_, i) => {
          ...
          return (
            <line
              key={i}
              x1={150 + 127 * Math.cos(a)}
              y1={150 + 127 * Math.sin(a)}
              x2={150 + 131 * Math.cos(a)}
              y2={150 + 131 * Math.sin(a)}
              stroke={`${accent}66`}
              strokeWidth="1.5"
            />
          );
...
function ShopLocator() {
  ...
  const fetchShops = useCallback(async (lat, lng, rad) => {
    ...
    try {
      ...
      const results = (data.elements || [])
        .map((el) => ({
          id: el.id,
          name: el.tags?.name || "Tyre / Auto Shop",
      ...
      setErrMsg(
        "All map servers failed. Please check your connection and try again.",
      );
      setStatus("error");
    }
  }, []);
  ...
  const locate = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        setCoords({ lat, lng });
        fetchShops(lat, lng, radius);
      },
      (err) => {
        if (err.code === 1) setStatus("denied");
        else if (err.code === 2) {
          setErrMsg("Location unavailable. Make sure GPS is enabled.");
          setStatus("error");
        } else {
    ...
    return (
      ...
      >
        ...
        <div>
          ...
          >
            Based on your diagnosis, we recommend visiting a professional. Share
            your location to find the nearest tyre shops.
          </p>
        </div>
        <button
          onClick={(e) => {
          ...
          className="mag-btn touch-btn"
          style={{
            background: `linear-gradient(135deg,${T.accentMid},${T.accentDark})`,
            border: "none",
            color: "white",
            fontFamily: "'Syne',sans-serif",
            fontWeight: 700,
  ...
  if (status === "loading")
    return (
      <LocatorStatus
        icon="⌁"
        msg="Searching for tyre shops…"
        sub={`Looking within ${radius} km radius.`}
        spin
      />
    );
  if (status === "denied")
    return (
      <LocatorStatus
        ...
        sub="Go to browser settings → Site permissions → Location → Allow, then try again."
        err
        onRetry={locate}
      />
    );
          ...
          {RADIUS_OPTIONS.map((km) => (
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                transition: "all .2s",
                background: radius === km ? `${T.accent}33` : T.ghost,
                border:
                  radius === km
                    ? `1px solid ${T.accent}80`
                    : `1px solid ${T.border}`,
                color: radius === km ? T.accent : T.textMuted,
                position: "relative",
                overflow: "hidden",
              }}
            >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {shops.map((shop) => (
          <ShopCard
            ...
            onClick={() => {
              haptic("light");
              setSelected((s) => (s?.id === shop.id ? null : shop));
            }}
          />
        ))}
      </div>
...
function addMarkers(L, map, layer, coords, shops) {
  ...
  const userIcon = L.divIcon({
    className: "",
    html: `<div style="width:14px;height:14px;border-radius:50%;background:#f97316;border:2px solid white;box-shadow:0 0 12px rgba(249,115,22,0.7)"></div>`,
    iconSize: [14, 14],
    iconAnchor: [7, 7],
  });
  ...
  shops.forEach((shop) => {
    const shopIcon = L.divIcon({
      className: "",
      html: `<div style="width:10px;height:10px;border-radius:50%;background:#fff;border:2px solid #f97316;box-shadow:0 0 8px rgba(0,0,0,0.4)"></div>`,
      iconSize: [10, 10],
      iconAnchor: [5, 5],
    });
    L.marker([shop.lat, shop.lng], { icon: shopIcon })
      .addTo(layer)
      .bindPopup(
        `<b style="font-size:12px">${shop.name}</b>${shop.addr ? `<br><span style="font-size:10px;color:#666">${shop.addr}</span>` : ""}`,
      );
  });
}
  ...
  return (
    <div
      ...
      style={{
        ...
        cursor: "pointer",
        borderLeft: selected
          ? `2px solid ${T.accent}99`
          : `2px solid ${T.borderFaint}`,
        transition: "all .25s",
        background: selected ? `${T.accent}0a` : G.card.background,
        position: "relative",
        overflow: "hidden",
      }}
    >
        ...
        {spin ? (
          ...
          <div
            style={{
              ...
              height: "100%",
              borderRadius: "50%",
              background: err ? "rgba(239,68,68,0.1)" : `${T.accent}14`,
              border: `1px solid ${err ? "rgba(239,68,68,0.25)" : T.accent + "33"}`,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              fontSize: 18,
    ...
    () => [
      {
        icon: "◎",
        n: "01",
        title: "Wear Classification",
        desc: "EfficientNet-B3 classifies wear across 5 levels — New to Bald — with sub-millimetre pattern sensitivity.",
      },
      {
        icon: "▦",
        n: "02",
        title: "Tread Depth Estimation",
        desc: "Computer vision estimates remaining depth in mm and predicts remaining kilometres before legal minimum.",
      },
      {
        icon: "⌁",
        n: "03",
        title: "Sidewall Detection",
        desc: "YOLOv8 detects bulges, cuts, and dry rot — sidewall damage invisible to the untrained eye.",
      },
      {
        icon: "◈",
        n: "04",
        title: "Grad-CAM Heatmaps",
        desc: "Gradient attention maps reveal exactly which tread zones drove the AI verdict. Full explainability.",
      },
      {
        icon: "◐",
        n: "05",
        title: "Pattern Diagnosis",
        desc: "Identifies cupping, feathering, one-sided wear — each linked to specific mechanical root causes.",
      },
      {
        icon: "◑",
        n: "06",
        title: "Health Score",
        desc: "All module outputs synthesised into one composite A–F grade with urgency level and recommendation.",
      },
    ],
    [],
  );
    ...
    <div>
      <section
        ...
        style={{
          ...
          textAlign: "center",
          padding:
            "clamp(80px,14vh,120px) clamp(16px,5vw,40px) clamp(40px,6vh,60px)",
          position: "relative",
        }}
      >
        <div
        ...
        >
          ...
          />
          <span
            style={{ fontSize: 9, color: T.textSub, letterSpacing: "0.12em" }}
          >
            AI-POWERED TYRE INTELLIGENCE
          </span>
          <span
            style={{
        ...
        <div className="hero-title-wrap">
          <h1
            className={`hero-title ${theme === "dark" ? "shimmer-text" : "shimmer-text-light"}`}
            style={{
              fontFamily: "'Syne',sans-serif",
              fontWeight: 800,
        ...
        <p
          className="hero-desc hero-desc-wrap"
          style={{
        ...
        >
          Upload tyre photos. Get a full AI diagnostic report in seconds — wear
          level, tread depth, pattern analysis, and explainable heatmaps.
        </p>
        <div
          className="hero-buttons hero-buttons-wrap"
          style={{
            display: "flex",
            gap: 12,
            justifyContent: "center",
        ...
        >
          <button
            ...
            className="mag-btn btn-primary-shadow touch-btn"
            style={{
              background: `linear-gradient(135deg,${T.accentMid},${T.accentDark})`,
              border: "none",
              color: "white",
      ...
      <section
        style={{
          maxWidth: 1100,
          margin: "0 auto",
          padding: "clamp(48px,8vh,120px) clamp(16px,5vw,40px)",
        }}
      >
        <div
          className="strada-reveal"
          style={{ textAlign: "center", marginBottom: "clamp(36px,6vh,80px)" }}
        >
          <p
            style={{
              fontSize: 10,
          ...
          >
            Six AI Engines.
            <br />
            <span style={{ color: T.textMuted }}>One Verdict.</span>
          </h2>
        </div>
        <div className="grid3">
          {features.map((f, i) => (
            <div
              key={i}
              className="strada-reveal lift-card touch-card"
              style={{
                ...G.card,
                borderRadius: 18,
            ...
            >
              ...
              <div style={{ fontSize: 20, marginBottom: 12, color: T.accent }}>
                {f.icon}
              </div>
              <h3
                style={{
      ...
      <section
        ...
        style={{
          maxWidth: 900,
          margin: "0 auto clamp(80px,12vh,130px)",
        ...
        >
          <div
            style={{
              ...
              width: "40%",
              height: "100%",
              background: `linear-gradient(135deg,transparent,${T.accent}0a)`,
              pointerEvents: "none",
            }}
          />
          <p
          ...
          >
            <div
              ...
              style={{
                ...
                right: 32,
                height: 1,
                background: `linear-gradient(90deg,${T.accent}66,${T.accent}1a,transparent)`,
              }}
            />
            {[
              "Upload 5 Images",
              ...
              >
                <div
                  style={{
                    ...
                    background:
                      i === 0
                        ? `linear-gradient(135deg,${T.accentMid},${T.accentDark})`
                        : G.card.background,
                    border: `1px solid ${T.accent}4d`,
                    display: "flex",
                    alignItems: "center",
                    ...
                    flexShrink: 0,
                    boxShadow:
                      i === 0 ? "0 0 18px rgba(249,115,22,0.4)" : "none",
                    position: "relative",
                    zIndex: 1,
                  }}
                >
    ...
    () => [
      {
        n: "01",
        title: "Upload Images",
        desc: "Provide up to 5 photos — left/right profiles, area of interest, tread close-up, and optional sidewall crack scan for maximum coverage.",
      },
      {
        n: "02",
        title: "Multi-Model Inference",
        desc: "EfficientNet-B3, CNN pattern classifier, YOLOv8, and EasyOCR (for DOT code) run simultaneously on your uploaded images.",
      },
      {
        n: "03",
        title: "Grad-CAM Explained",
        desc: "A backward pass generates gradient-weighted attention maps highlighting exactly which tread regions drove the wear classification verdict.",
      },
      {
        n: "04",
        title: "Health Score Synthesis",
        desc: "All outputs aggregated into a composite 0–100 health score graded A–F, with urgency level (low/medium/high) and plain-English recommendation.",
      },
      {
        n: "05",
        title: "Full Diagnostic Report",
        desc: "A printable report with gauges, depth bars, score breakdowns, Grad-CAM image, submitted photos, and quality warnings is instantly generated.",
      },
    ],
    [],
  );
  return (
    <div
      style={{
        ...
        margin: "0 auto",
        padding:
          "clamp(76px,10vh,120px) clamp(16px,5vw,40px) clamp(80px,10vh,80px)",
      }}
    >
      <div className="strada-reveal" style={{ marginBottom: 56 }}>
        ...
        >
          THE DIAGNOSTIC
          <br />
          <span style={{ color: T.textMuted }}>PIPELINE.</span>
        </h2>
        <p
          style={{
            fontSize: 13,
        ...
        >
          Strada runs a chain of computer vision models on your tyre images,
          each specialising in a distinct aspect of tyre health.
        </p>
      </div>
      <div
        style={{
        ...
        {steps.map((st, i) => (
          <div
            ...
            style={{
              ...
              gap: 20,
              alignItems: "flex-start",
              borderLeft: `1px solid ${T.accent}1e`,
            }}
          >
            <div
              style={{
      ...
      >
        ...
        >
          Strada is an AI diagnostic aid, not a replacement for professional
          tyre inspection. Always consult a certified technician before making
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          style={{
            ...
            fontWeight: 700,
            fontSize: 18,
            color: uploadedCount > 0 ? T.accent : T.textFaint,
          }}
        >
          {uploadedCount}
          <span
      ...
      >
        ...
        >
          <span style={{ color: `${T.accent}b3`, fontWeight: 600 }}>Tip:</span>{" "}
          Use flash, place a coin in the tread groove to help the AI calibrate
          depth.
        </p>
      </div>
      <div
          ...
          return (
            <SlotTile
              ...
              onDrop={(e) => {
                ...
                if (f?.type.startsWith("image/")) onUpload(slot.id, f);
              }}
  ...
  return (
    <div
      ...
      style={{
        ...
        ...(isDragging
          ? { background: `${T.accent}14`, border: `1.5px solid ${T.accent}99` }
          : file
            ? {
        ...
        >
          <div
            style={{
              ...
              alignItems: "center",
              justifyContent: "center",
              background: isDragging ? `${T.accent}1e` : T.ghost,
              flexShrink: 0,
            }}
          >
            <span
          ...
          <div style={{ textAlign: "center" }}>
            <p
              style={{
                fontSize: 10,
                color: isDragging ? T.accent : T.textSub,
                margin: "0 0 2px",
                letterSpacing: "0.04em",
              }}
            >
...
const DIAGNOSTIC_STEPS = [
  {
    id: "wear",
    label: "Classifying wear level",
    detail: "EfficientNet-B3 running…",
    duration: 950,
  },
  {
    id: "pattern",
    label: "Analysing wear pattern",
    detail: "CNN pattern classifier…",
    duration: 850,
  },
  {
    id: "depth",
    label: "Estimating tread depth",
    detail: "Depth prediction model…",
    duration: 1050,
  },
  {
    id: "sidewall",
    label: "Scanning sidewall damage",
  ...
  },
  {
    ...
    detail: "Gradient attention map…",
    duration: 1100,
  },
  {
    id: "score",
    label: "Synthesising health score",
    detail: "Aggregating all outputs…",
    duration: 700,
  },
];
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            {completedSteps.length}/{DIAGNOSTIC_STEPS.length} modules complete
          </p>
        </div>
        <div style={{ display: "flex", gap: 3, flexShrink: 0 }}>
          {DIAGNOSTIC_STEPS.map((s, i) => (
            <div
              key={s.id}
              style={{
                width: completedSteps.includes(s.id)
                  ? 12
                  : activeStep === i
                    ? 8
                    : 4,
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {DIAGNOSTIC_STEPS.map((step, i) => {
          const done = completedSteps.includes(step.id),
            active = activeStep === i && !done;
          return (
            <div
              key={step.id}
              className={done || active ? "status-anim" : ""}
              style={{
                display: "flex",
                alignItems: "center",
                gap: 12,
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p
                  style={{
                    fontSize: 11,
                    margin: "0 0 1px",
                    color: done ? T.textMuted : active ? T.text : T.textFaint,
                    fontFamily: active
                      ? "'Syne',sans-serif"
              ...
              <span
                style={{
                  ...
                  letterSpacing: "0.1em",
                  flexShrink: 0,
                  color: done ? "#10b981" : active ? T.accent : T.textFaint,
                }}
              >
...
function ScoreBar({ label, score, max, sublabel }) {
  const pct = (score / max) * 100,
    c = pct >= 70 ? "#10b981" : pct >= 40 ? "#f59e0b" : "#ef4444",
    T = useTokens();
  return (
...
function DepthMeter({ depth_mm, status, color, remaining_km }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      >
        <span>0mm</span>
        <span style={{ color: `${T.accent}8c` }}>▲ 1.6 legal min</span>
        <span>9mm new</span>
      </div>
      {remaining_km != null && (
        <p style={{ fontSize: 11, color: T.textMuted, margin: 0 }}>
          ~{remaining_km.toLocaleString()} km remaining
        </p>
      )}
    </div>
  );
...
function EditableTyreAgeCard({ tyreAge, onChange }) {
  const [editing, setEditing] = useState(false),
    [value, setValue] = useState(tyreAge?.age_display || "Unknown"),
  ...
  return (
    <div
      style={{
        ...
        borderRadius: 12,
        padding: "14px 16px",
        borderLeft: `2px solid ${isHighlight ? "rgba(239,68,68,0.4)" : T.borderFaint}`,
      }}
    >
      <div
        style={{
      ...
      >
        ...
        <button
          ...
          style={{
            background: editing ? `${T.accent}26` : T.ghost,
            border: editing
              ? `1px solid ${T.accent}4d`
              : `1px solid ${T.border}`,
            color: editing ? T.accent : T.textMuted,
            fontSize: 9,
            letterSpacing: "0.1em",
            padding: "5px 10px",
            borderRadius: 5,
      ...
      {editing ? (
        <input
          ...
          onKeyDown={(e) => {
            ...
            if (e.key === "Escape") {
              setEditing(false);
              setValue(tyreAge?.age_display || "Unknown");
            }
          }}
          style={{
            width: "100%",
        ...
        <p
          style={{
            ...
            fontFamily: "'Syne',sans-serif",
            fontWeight: 700,
            color: isHighlight ? "#ef4444" : T.text,
            margin: "0 0 3px",
          }}
        >
          {value}
      ...
      >
        {tyreAge?.manufacture || ""}
        {!tyreAge?.dot_found ? " (DOT not detected)" : ""}
      </p>
    </div>
  );
}
  ...
  return (
    <div
      style={{
        ...
        borderRadius: 12,
        padding: "14px 16px",
        borderLeft: `2px solid ${highlight ? "rgba(239,68,68,0.4)" : T.borderFaint}`,
      }}
    >
      <p
        style={{
...
function GradCamDisplay({ base64, originalBase64 }) {
  ...
  return (
    ...
    >
      {originalBase64 && (
        <img
          src={`data:image/jpeg;base64,${originalBase64}`}
          alt="Original tread"
          style={{
            width: "100%",
            objectFit: "contain",
      ...
      <img
        ...
        alt="Grad-CAM"
        className={originalBase64 ? "gradcam-img" : ""}
        style={{
          width: "100%",
          objectFit: "contain",
          maxHeight: 240,
      ...
      >
        {originalBase64 ? "OVERLAY" : "HEATMAP"}
      </div>
    </div>
  );
}
...
function PrintReport({ result, previews }) {
  ...
  if (!portalNode.current) {
    ...
    node.style.cssText =
      "display:none;position:fixed;left:-99999px;top:0;visibility:hidden";
  ...
  const depthBarColor =
    depthPct >= 60 ? "#059669" : depthPct >= 25 ? "#d97706" : "#dc2626";
  const healthColor =
    health.color === "green"
      ? "#059669"
      : health.color === "yellow"
  ...
  const content = (
    <>
      <div
        ...
        style={{ padding: "0 0 24pt", position: "relative" }}
      >
        <div
          style={{
            display: "flex",
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "10pt" }}>
            ...
            <div>
              <p className="pt-h1" style={{ fontSize: "22pt" }}>
                STRADA
              </p>
              <p className="pt-label" style={{ marginBottom: 0 }}>
                AI TYRE DIAGNOSTIC REPORT
              </p>
            </div>
          </div>
          <div style={{ textAlign: "right" }}>
            <p className="pt-small" style={{ margin: 0, fontWeight: 700 }}>
              Report ID: {reportId}
            </p>
            <p className="pt-small" style={{ margin: "3pt 0 0" }}>
              {now} at {time}
            </p>
            <p
              className="pt-small"
              style={{ margin: "2pt 0 0", color: "#aaa" }}
            >
        ...
        <div className={urgencyBadgeClass} style={{ marginBottom: "12pt" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            >
              {result.recommendation || "See full report for details."}
            </span>
          </div>
        </div>
        <div
          className="pt-kpi-grid print-no-break"
        ...
        >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "10pt" }}>
              ...
              >
                ...
                <circle
                  ...
                  stroke={healthColor}
                  strokeWidth="5"
                  strokeDasharray={`${((health.score ?? 0) / 100) * 157.1} 157.1`}
                  strokeLinecap="round"
                />
              </svg>
              <div>
                <p
                  className="pt-value"
                  style={{ color: healthColor, fontSize: "24pt" }}
                >
                  {health.grade || "—"}
                </p>
                <p className="pt-small">{health.score ?? "—"}/100</p>
                <p className="pt-small" style={{ fontStyle: "italic" }}>
                  {health.label || ""}
                </p>
              </div>
            </div>
          ...
          <div className="pt-kpi">
            ...
            <p className="pt-value" style={{ color: depthBarColor }}>
              {depth.depth_mm ?? "—"}
              <span
                style={{ fontSize: "11pt", fontWeight: 400, color: "#666" }}
              >
                {" "}
                mm
              </span>
            </p>
            <p className="pt-small" style={{ marginBottom: "5pt" }}>
              {depth.status || ""}
            </p>
            ...
            <p className="pt-small" style={{ marginTop: "4pt" }}>
              Min: 1.6 mm
              {depth.remaining_km != null
                ? ` · ~${depth.remaining_km.toLocaleString()} km left`
                : ""}
            </p>
          </div>
          <div className="pt-kpi">
            ...
            <p
              className="pt-value"
              style={{ fontSize: "16pt", color: urgencyTextColor }}
            >
              {u.label}
            </p>
            <p
              className="pt-small"
              style={{ marginTop: "4pt", lineHeight: 1.5 }}
            >
              {result.urgency === "high"
                ? "Replace immediately — unsafe to drive."
                : result.urgency === "medium"
                  ? "Replace within 2 weeks / 1,000 km."
                  : "Monitor at next service interval."}
            </p>
          </div>
        </div>
        <div
          className="pt-4col print-no-break"
          style={{ marginBottom: "10pt" }}
        >
          <div className="pt-kpi-2">
            ...
            <p
              style={{
                ...
                fontWeight: 700,
                fontSize: "11pt",
                color: result.urgency === "high" ? "#dc2626" : "#111",
                margin: "4pt 0 2pt",
              }}
            >
              {result.wear_level || "—"}
          ...
          <div className="pt-kpi-2">
            ...
            >
              {result.tyre_age?.age_display || "Unknown"}
            </p>
            <p className="pt-small">
              {result.tyre_age?.manufacture || ""}
              {!result.tyre_age?.dot_found ? " (DOT not detected)" : ""}
            </p>
          </div>
          <div className="pt-kpi-2">
        ...
        <div className="pt-reco print-no-break">
          <p
            className="pt-h3"
            style={{ color: "#ea580c", marginBottom: "5pt" }}
          >
            WORKSHOP RECOMMENDATION
          </p>
          <p className="pt-body">
            {result.recommendation ||
              "Consult a qualified tyre technician for a full physical inspection."}
          </p>
        </div>
        {result.warnings?.length > 0 && (
          <div className="pt-warn print-no-break">
            <p
              className="pt-h3"
              style={{ color: "#b45309", marginBottom: "5pt" }}
            >
              AI QUALITY FLAGS
            </p>
      ...
      <div className="print-page print-break-before">
        ...
        >
          <p className="pt-h2" style={{ margin: 0 }}>
            SCORE BREAKDOWN
          </p>
        ...
        <div className="print-no-break" style={{ marginBottom: "14pt" }}>
          {Object.keys(breakdown).length > 0 ? (
            ...
            <p className="pt-small" style={{ margin: "0 0 14pt" }}>
              Score breakdown not available.
            </p>
          )}
        </div>
        <div
          className="pt-2col print-no-break"
          style={{ marginBottom: "14pt" }}
        >
          <div>
            <p className="pt-h3" style={{ marginBottom: "6pt" }}>
            ...
            {result.gradcam_image ? (
              ...
              >
                <p className="pt-small">Not available</p>
              </div>
            )}
            <p
              className="pt-small"
              style={{ marginTop: "4pt", fontStyle: "italic" }}
            >
              Highlighted zones indicate regions driving the AI verdict.
            </p>
          </div>
          <div>
            <p className="pt-h3" style={{ marginBottom: "6pt" }}>
        ...
        <div className="pt-checklist print-no-break">
          <p className="pt-h3" style={{ marginBottom: "8pt" }}>
          ...
          {[
            ...
            >
              ...
              />
              <p className="pt-body" style={{ margin: 0 }}>
                {item}
              </p>
            </div>
          ))}
        ...
        >
          ...
          >
            <strong style={{ color: "#666" }}>DISCLAIMER:</strong> This report
            is generated by the STRADA AI Tyre Intelligence system and is
            intended as a supplementary diagnostic aid only. Report ID:{" "}
            {reportId}
          </p>
        </div>
        <div className="pt-footer">
...
function ReportPage({ result, previews, onClose }) {
  ...
  const handleTouchEnd = (e) => {
    ...
    if (dy > 80 && overlayRef.current?.scrollTop === 0) {
      haptic("medium");
      onClose();
    } else {
    ...
    >
      ...
      <div
        ...
        style={{
          maxWidth: 760,
          margin: "0 auto",
          padding: `clamp(20px,5vh,64px) clamp(14px,4vw,28px) ${isMobile ? "90px" : "48px"}`,
        }}
      >
        {isMobile && (
          <div
        ...
        >
          ...
          <span
            style={{ fontSize: 12, color: T.textSub, flex: 1, minWidth: 160 }}
          >
            {result.recommendation}
          </span>
        </div>
        ...
        >
          <div className="card-anim" style={{ animationDelay: "0.22s" }}>
            <MiniCard
              label="WEAR LEVEL"
              value={result.wear_level}
              sub={result.cause}
              highlight={result.urgency === "high"}
            />
          </div>
          <div className="card-anim" style={{ animationDelay: "0.27s" }}>
            <MiniCard
              label="WEAR PATTERN"
          ...
          <div className="card-anim" style={{ animationDelay: "0.32s" }}>
            <EditableTyreAgeCard
              tyreAge={tyreAge}
              onChange={(val) =>
                setTyreAge((prev) => ({ ...prev, age_display: val }))
              }
            />
          </div>
          <div className="card-anim" style={{ animationDelay: "0.37s" }}>
            <MiniCard
              label="SIDEWALL"
              value={result.sidewall}
              sub={
                result.sidewall === "None"
                  ? "No damage detected"
                  : "⚠ Damage detected"
              }
              highlight={result.sidewall !== "None"}
            />
          </div>
        </div>
        <div
        ...
        >
          ...
          <p style={{ fontSize: 10, color: T.textFaint, margin: "10px 0 0" }}>
            Highlighted regions indicate areas the model focused on during
            classification.
          </p>
        </div>
        {previews.length > 0 && (
        ...
        >
          <span
            style={{ fontSize: 9, color: T.textFaint, letterSpacing: "0.1em" }}
          >
            STRADA · TYRE INTELLIGENCE
          </span>
          <span
            style={{ fontSize: 9, color: T.textFaint, letterSpacing: "0.08em" }}
          >
...
function ResultCard({ result, onViewReport }) {
  ...
  return (
    ...
    >
      ...
      >
        <span
          style={{ fontSize: 9, color: T.textFaint, letterSpacing: "0.18em" }}
        >
          DIAGNOSTIC RESULT
        </span>
        <div
      ...
      >
        <div className="card-anim" style={{ animationDelay: "0.14s" }}>
          <MiniCard
            label="WEAR LEVEL"
            value={result.wear_level}
            highlight={result.urgency === "high"}
        ...
        <div className="card-anim" style={{ animationDelay: "0.18s" }}>
          <MiniCard label="PATTERN" value={result.pattern} />
        </div>
        <div className="card-anim" style={{ animationDelay: "0.22s" }}>
          <EditableTyreAgeCard
            tyreAge={tyreAge}
        ...
        <div className="card-anim" style={{ animationDelay: "0.26s" }}>
          <MiniCard
            label="SIDEWALL"
            value={result.sidewall}
            highlight={result.sidewall !== "None"}
      ...
      <div
        style={{
          ...
          padding: "12px 16px",
          marginBottom: 14,
          borderLeft: `2px solid ${T.accent}38`,
        }}
      >
        <p
          style={{
...
function DiagnosePage({ isMobile, onSlickEgg }) {
  ...
  const previews = useMemo(
    () =>
      SLOTS.filter((s) => files[s.id]).map((s) => ({
        label: s.label,
        url: URL.createObjectURL(files[s.id]),
      })),
    [files],
  );
  useEffect(
  ...
  const handleAnalyse = async () => {
    ...
    SLOTS.forEach((s) => {
      if (files[s.id]) fd.append(s.id, files[s.id]);
    });
    try {
      ...
      const wearLow =
        typeof data.wear_level === "string" &&
        data.wear_level.toLowerCase().includes("bald");
      if (data.urgency === "high" || wearLow) {
        setTimeout(() => onSlickEgg?.(), 800);
      }
    } catch (err) {
      if (err.name === "AbortError")
        setError("Request timed out. Check that your Flask server is running.");
      else
        setError(
          err.message || "Failed to reach API. Is Flask running on port 5000?",
        );
      haptic("heavy");
    } finally {
      setLoading(false);
  ...
  return (
    <div
      style={{
        maxWidth: 720,
        margin: "0 auto",
        padding: `clamp(72px,10vh,106px) clamp(14px,5vw,28px) ${isMobile ? "90px" : "80px"}`,
      }}
    >
      {showReport && result && (
        <ReportPage
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        >
          Upload photos for AI wear analysis. At least 1 image required — 5
          images gives the most accurate result.
        </p>
      </div>
      <UnifiedUploadCard
        files={files}
      ...
      <button
        ...
        className={canAnalyse && !loading ? "mag-btn touch-btn" : ""}
        style={{
          width: "100%",
          padding: "16px",
          borderRadius: 14,
          ...
          ...(canAnalyse && !loading
            ? {
                ...
                color: "white",
                boxShadow:
                  "0 0 40px rgba(249,115,22,0.35),0 6px 24px rgba(0,0,0,0.15)",
              }
            : {
                background: T.ghost,
                border: `1px solid ${T.border}`,
      ...
      {result && !loading && (
        <ResultCard result={result} onViewReport={() => setShowReport(true)} />
      )}
      <div
        style={{
          marginTop: 48,
...
export default function App() {
  ...
  return (
    <ThemeContext.Provider value={{ theme, toggleTheme }}>
      ...
      {showRollingTyre && (
        <RollingTyre onDone={() => setShowRollingTyre(false)} />
      )}

      {/* ── Easter Egg: Racing Slick popup for bald tyres */}
      {showSlickPopup && (
        <RacingSlickPopup onClose={() => setShowSlickPopup(false)} />
      )}
      ...
      >
        ...
        <div style={{ position: "relative", zIndex: 2 }}>
          {page === "landing" && (
            <LandingPage
              setPage={go}
              onRadarTripleClick={handleRadarTripleClick}
            />
          )}
          {page === "diagnose" && (
            <DiagnosePage isMobile={isMobile} onSlickEgg={handleSlickEgg} />
          )}
          {page === "info" && <InfoPage />}
        ...
        {!isMobile && (
          ...
          >
            ...
            >
              AI TYRE INTELLIGENCE · NOT A SUBSTITUTE FOR PROFESSIONAL
              INSPECTION
            </span>
